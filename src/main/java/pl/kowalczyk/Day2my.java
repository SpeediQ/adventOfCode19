...
public class Day2my {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        System.out.println(nextTry(fileToList()));
...
//        outList.set(2, 2);
    ...
    private static List<Integer> nextTry(List<Integer> inList) {

        List<Integer> copyOfInList = copyList(inList);


        List<Integer> list = modifiedList(copyOfInList);
        ...
        while (valueVerb < 79 && !(modifiedList(copyOfInList)).get(0).equals(wantedValue)) {
            try {
                valueVerb++;
                copyOfInList.set(2, valueVerb);
                if (!(modifiedList(copyOfInList)).get(0).equals(wantedValue)) {
                    for (valueNoun = 0; valueNoun <= 53; valueNoun++) {
                        copyOfInList.set(1,valueNoun);
                        if ((modifiedList(copyOfInList)).get(0).equals(wantedValue)){
        ...
        return modifiedList(copyOfInList);
    }
